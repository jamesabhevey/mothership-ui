...
Every commit on `main` is a version. There is no release step: a change lands,
the Pages deploy runs, and that is what anyone pulling the library gets — so
there is no window in which several changes could be gathered into a release.

Names are calendar-based: the day it landed plus which change of that day it
was, counted from the first. `2026.09.10.2` is the second change on 10
September. Not semver, because nothing here is published as a package — there is
no install to pin to, so a number counting breaking changes would be describing
...
from the git history at build time; see the note in that file for why it is not
fetched from the GitHub API in the browser.
